...
impl AWSConf {
    pub fn convert(&self) -> AWS {
        AWS::new(&self.application, &self.environment, &self.configuration, 
                 &self.client_id, &self.state_file)
    }
}
...
impl AWS {
    ...
    pub fn new(application: &str, environment: &str, 
               configuration: &str, client_id: &str, 
               state_file: &Option<String>) -> AWS {
    ...
    fn pull_latest_version(db_conn: &Connection) -> rusqlite::Result<isize> {
        let res: isize = db_conn.query_row(
                        "SELECT version FROM appConfig WHERE id=0", params![],
                        |row| row.get(0), )?;
        Ok(res)
    }
...
impl Provider for AWS {
    ...
    fn poll(&self) -> Option<String> {
        ...
        let data = std::str::from_utf8(&configuration.content.unwrap()).unwrap().to_string();
    ...
    fn query(&self) -> BoxResult<String> {
        ...
        let res: String = self.db_conn.query_row(
                        "SELECT data FROM appConfig WHERE id=0", params![],
                        |row| row.get(0), )?;
        Ok(res)
    }
}
...
async fn get_config(request: GetConfigurationRequest) -> rusoto_appconfig::Configuration {
    ...
    match result {
        ...
        Err(e) => {
            eprintln!("An error occurred - {:?} - when trying to fetch configuration", e);
            std::process::exit(exitcode::UNAVAILABLE);
        },
    }
}
